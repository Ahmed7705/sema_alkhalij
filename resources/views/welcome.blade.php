<x-app-layout>
    ...
    <section class="relative h-[85vh] min-h-[550px] max-h-[800px] overflow-hidden" x-data="{ slide: 1 }" x-init="setInterval(() => slide = slide === 3 ? 1 : slide + 1, 6000)">
        ...
        <div class="absolute inset-0">
            <div class="absolute inset-0 transition-all duration-[1500ms] ease-in-out"
                ...
                <img src="{{ asset('images/hero-doctor.png') }}" alt="{{ __('hero.slide1_title') }}" class="w-full h-full object-cover" style="object-position: center 20%;">
            </div>
            <div class="absolute inset-0 transition-all duration-[1500ms] ease-in-out"
                 :class="slide === 2 ? 'opacity-100 scale-100' : 'opacity-0 scale-110'">
                <img src="{{ asset('images/nurse-care.png') }}" alt="{{ __('hero.slide2_title') }}" class="w-full h-full object-cover" style="object-position: center 20%;">
            </div>
            <div class="absolute inset-0 transition-all duration-[1500ms] ease-in-out"
                 :class="slide === 3 ? 'opacity-100 scale-100' : 'opacity-0 scale-110'">
                <img src="{{ asset('images/physio-session.png') }}" alt="{{ __('hero.slide3_title') }}" class="w-full h-full object-cover" style="object-position: center 20%;">
            </div>
        ...
        <div class="relative z-10 h-full flex flex-col justify-center">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full pb-24 {{ app()->getLocale()=='en' ? 'text-left' : 'text-right' }}">
                ...
                <p class="text-white/90 text-sm sm:text-base font-bold mb-2">{{ __('hero.tagline') }}</p>

                <h1 class="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-black text-white leading-[1.1] tracking-tight mb-4 grid grid-cols-1 grid-rows-1">
                    <span x-show="slide === 1" x-transition.opacity.duration.300ms class="col-start-1 row-start-1">{{ __('hero.slide1_title') }}</span>
                    <span x-show="slide === 2" x-transition.opacity.duration.300ms class="col-start-1 row-start-1" x-cloak>{{ __('hero.slide2_title') }}</span>
                    <span x-show="slide === 3" x-transition.opacity.duration.300ms class="col-start-1 row-start-1" x-cloak>{{ __('hero.slide3_title') }}</span>
                </h1>

                <p class="text-lg sm:text-xl font-black mb-6 grid grid-cols-1 grid-rows-1">
                    <span x-show="slide === 1" x-transition.opacity.duration.300ms class="col-start-1 row-start-1 drop-shadow-md" style="color: #FCD34D !important;">{{ __('hero.slide1_sub') }}</span>
                    <span x-show="slide === 2" x-transition.opacity.duration.300ms class="col-start-1 row-start-1 drop-shadow-md" style="color: #FCD34D !important;" x-cloak>{{ __('hero.slide2_sub') }}</span>
                    <span x-show="slide === 3" x-transition.opacity.duration.300ms class="col-start-1 row-start-1 drop-shadow-md" style="color: #FCD34D !important;" x-cloak>{{ __('hero.slide3_sub') }}</span>
                </p>
                ...
                <div class="flex flex-wrap gap-3 mb-4">
                    <button @click="selectedService = ''; callbackModalOpen = true" class="inline-flex items-center gap-2 px-7 py-3.5 bg-white text-primary font-black text-sm rounded-xl shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300">
                        <svg class="w-4 h-4 text-accent" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        {{ __('hero.cta_service') }}
                    </button>
        ...
        <div class="absolute bottom-4 left-0 right-0 z-20 px-4 sm:px-6 lg:px-8">
            <div class="max-w-4xl mx-auto bg-white/95 backdrop-blur-lg rounded-2xl p-3 sm:p-4 shadow-[0_10px_30px_rgba(0,0,0,0.18)] border border-white/80">
                <div class="flex flex-col sm:flex-row items-stretch gap-3">
                    <div class="flex-1">
                        <label class="block text-[10px] font-bold text-gray-500 mb-1 px-1">{{ __('hero.search_city') }}</label>
                        <select class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-700 focus:outline-none focus:border-accent transition-all cursor-pointer">
                            <option>{{ __('hero.city_jeddah') }}</option>
                        </select>
                    </div>
                    <div class="flex-1">
                        <label class="block text-[10px] font-bold text-gray-500 mb-1 px-1">{{ __('hero.search_service') }}</label>
                        <select class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-700 focus:outline-none focus:border-accent transition-all cursor-pointer">
                            <option>{{ __('hero.search_all') }}</option>
                            <option>{{ __('hero.search_nursing') }}</option>
                            <option>{{ __('hero.search_doctor') }}</option>
                            <option>{{ __('hero.search_physio') }}</option>
                            <option>{{ __('hero.search_lab') }}</option>
                        </select>
                    </div>
    ...
    <section class="py-12 lg:py-16 bg-white relative overflow-hidden">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 items-center">
                <div class="relative">
                    <div class="relative">
                        <img src="{{ asset('images/medical-team.png') }}" alt="{{ __('about.badge') }}" class="w-full rounded-2xl shadow-card object-cover aspect-[4/3]">
                        <div class="absolute -z-10 -top-3 -right-3 w-full h-full rounded-2xl bg-accent/10 border-2 border-accent/20"></div>
                    </div>
                    <div class="absolute -bottom-4 {{ app()->getLocale()=='en' ? 'right-4' : 'left-4' }} bg-white py-3 px-5 rounded-xl shadow-floating border border-gray-100 text-right">
                        <span class="block text-3xl font-black text-accent leading-none">+10</span>
                        <span class="text-[11px] font-bold text-gray-600 mt-0.5 block">{{ __('about.years') }}</span>
                    </div>
                    <div class="absolute -top-3 {{ app()->getLocale()=='en' ? 'right-3' : 'left-3' }} bg-primary text-white py-2.5 px-4 rounded-xl shadow-floating">
                        ...
                        <span class="text-[10px] text-medical-200 block">{{ __('about.visits') }}</span>
                    </div>
                </div>
                <div class="space-y-4">
                    ...
                    <h2 class="text-2xl sm:text-3xl font-black text-primary leading-tight">{{ __('about.heading') }}</h2>
                    <p class="text-gray-600 text-sm leading-[1.8]">{{ __('about.text') }}</p>
                    <div class="grid grid-cols-3 gap-3">
                        <div class="text-center p-3 rounded-xl bg-surface border border-gray-100">
                            ...
                            <span class="text-[10px] text-gray-400 font-bold">{{ __('about.stat_satisfaction') }}</span>
                        </div>
                        <div class="text-center p-3 rounded-xl bg-surface border border-gray-100">
                            <span class="text-xl font-black text-accent block">+50</span>
                            <span class="text-[10px] text-gray-400 font-bold">{{ __('about.stat_staff') }}</span>
                        </div>
                        <div class="text-center p-3 rounded-xl bg-surface border border-gray-100">
                            <span class="text-xl font-black text-primary block">24/7</span>
                            <span class="text-[10px] text-gray-400 font-bold">{{ __('about.stat_coverage') }}</span>
                        </div>
                    </div>
                    <div class="flex flex-wrap gap-3 pt-1">
                        <a href="{{ url('/about') }}" class="btn-primary py-2.5 px-6 rounded-xl text-xs">{{ __('about.learn_more') }}
                            <svg class="w-3.5 h-3.5 {{ app()->getLocale()=='en' ? 'ml-1' : 'mr-1' }} inline" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="{{ app()->getLocale()=='en' ? 'M14 5l7 7m0 0l-7 7m7-7H3' : 'M10 19l-7-7m0 0l7-7m-7 7h18' }}"/></svg>
                        </a>
                        <a href="{{ url('/contact') }}" class="btn-outline py-2.5 px-6 rounded-xl text-xs">{{ __('about.contact_us') }}</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="py-12 lg:py-16 bg-surface">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center max-w-2xl mx-auto mb-10 space-y-2">
                ...
                <h2 class="text-2xl sm:text-3xl font-black text-primary">{{ __('services.heading') }}</h2>
                <p class="text-xs text-gray-500">{{ __('services.subtext') }}</p>
            </div>

            @php
            $services = [
                ['img'=>'service-care.png','icon'=>'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z','title'=>__('services.home_care'),'desc'=>__('services.home_care_desc'),'price'=>__('services.price_home_care'),'gradient'=>'from-emerald-950/85 via-primary/60 to-transparent'],
                ['img'=>'service-doctor.png','icon'=>'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z','title'=>__('services.home_visits'),'desc'=>__('services.home_visits_desc'),'price'=>__('services.price_visits'),'gradient'=>'from-teal-950/85 via-primary/60 to-transparent'],
                ['img'=>'service-nursing.png','icon'=>'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z','title'=>__('services.nursing'),'desc'=>__('services.nursing_desc'),'price'=>__('services.price_nursing'),'gradient'=>'from-cyan-950/85 via-primary/60 to-transparent'],
                ['img'=>'service-physio.png','icon'=>'M13 10V3L4 14h7v7l9-11h-7z','title'=>__('services.physio'),'desc'=>__('services.physio_desc'),'price'=>__('services.price_physio'),'gradient'=>'from-green-950/85 via-primary/60 to-transparent'],
                ['img'=>'service-sampling.png','icon'=>'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z','title'=>__('services.sampling'),'desc'=>__('services.sampling_desc'),'price'=>__('services.price_sampling'),'gradient'=>'from-emerald-950/85 via-primary/60 to-transparent'],
                ['img'=>'service-lab.png','icon'=>'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z','title'=>__('services.lab'),'desc'=>__('services.lab_desc'),'price'=>__('services.price_lab'),'gradient'=>'from-teal-950/85 via-primary/60 to-transparent'],
                ['img'=>'service-dna.png','icon'=>'M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A13.916 13.916 0 008 11a4 4 0 118 0c0 1.017-.07 2.019-.203 3m-2.118 6.844A21.88 21.88 0 0015.171 17m3.839 1.132c.645-2.266.99-4.659.99-7.132A8 8 0 008 4.07M3 15.364c.64-1.319 1-2.8 1-4.364 0-1.457-.39-2.823-1.07-4','title'=>__('services.genetics'),'desc'=>__('services.genetics_desc'),'price'=>__('services.price_genetics'),'gradient'=>'from-slate-950/85 via-primary/60 to-transparent'],
                ['img'=>'service-telehealth.png','icon'=>'M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z','title'=>__('services.teleconsult'),'desc'=>__('services.teleconsult_desc'),'price'=>__('services.price_teleconsult'),'gradient'=>'from-cyan-950/80 via-primary/60 to-transparent'],
                ['img'=>'medical-team.png','icon'=>'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4','title'=>__('services.corporate'),'desc'=>__('services.corporate_desc'),'price'=>__('services.price_corporate'),'gradient'=>'from-emerald-900/80 via-primary/60 to-transparent'],
            ];
            @endphp
                ...
                <div class="medical-card group border border-gray-100 hover:border-accent/40 shadow-soft hover:shadow-card transition-all duration-300 rounded-2xl overflow-hidden flex flex-col justify-between">
                    <div>
                        <div class="relative h-44 overflow-hidden">
                            ...
                            <span class="absolute bottom-3 {{ app()->getLocale()=='en' ? 'left-3' : 'right-3' }} px-3 py-1 bg-accent text-white text-[11px] font-bold rounded-lg shadow-md border border-white/20">{{ $service['price'] }}</span>
                            <div class="absolute top-3 {{ app()->getLocale()=='en' ? 'right-3' : 'left-3' }} w-9 h-9 rounded-xl bg-white/90 backdrop-blur-md text-primary flex items-center justify-center shadow-sm">
                    ...
                    <div class="px-5 pb-5 pt-2 flex items-center justify-between border-t border-gray-50 mt-2">
                        <a href="{{ url('/services') }}" class="text-xs font-bold text-primary hover:text-accent transition-colors flex items-center gap-1">
                            <span>{{ __('services.details') }}</span>
                            <svg class="w-3 h-3 text-accent {{ app()->getLocale()=='en' ? 'rotate-180' : '' }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
                        </a>
                        <button @click="selectedService = '{{ $service['title'] }}'; callbackModalOpen = true" class="btn-accent text-xs py-2 px-4 rounded-xl shadow-md hover:shadow-lg transition-all font-bold">{{ __('services.book_now') }}</button>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center max-w-2xl mx-auto mb-10 space-y-2">
                <div class="section-badge mx-auto">
                    ...
                    <span>{{ __('why.badge') }}</span>
                </div>
                <h2 class="text-2xl sm:text-3xl font-black text-primary">{{ __('why.heading') }}</h2>
            </div>

            @php
            $advantages = [
                ['icon'=>'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z','title'=>__('why.licensed_title'),'desc'=>__('why.licensed_desc')],
                ['icon'=>'M13 10V3L4 14h7v7l9-11h-7z','title'=>__('why.fast_title'),'desc'=>__('why.fast_desc')],
                ['icon'=>'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z','title'=>__('why.equipment_title'),'desc'=>__('why.equipment_desc')],
                ['icon'=>'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z','title'=>__('why.privacy_title'),'desc'=>__('why.privacy_desc')],
                ['icon'=>'M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z','title'=>__('why.partners_title'),'desc'=>__('why.partners_desc')],
                ['icon'=>'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z','title'=>__('why.pricing_title'),'desc'=>__('why.pricing_desc')],
            ];
            @endphp
    ...
    <section class="relative py-12 overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 text-center">
                @foreach([
                    ['+15,000', __('stats.visits')],
                    ['99.2%', __('stats.satisfaction')],
                    ['+50', __('stats.staff')],
                    ['24/7', __('stats.coverage')],
                    ['+120', __('stats.tests')],
                    ['100%', __('stats.certified')],
                ] as $stat)
                <div class="space-y-1">
    ...
    <section class="py-12 lg:py-16 bg-surface">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center max-w-2xl mx-auto mb-10 space-y-2">
                ...
                <h2 class="text-2xl sm:text-3xl font-black text-primary">{{ __('testimonials.heading') }}</h2>
            </div>
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center space-y-2 mb-10">
                <div class="section-badge mx-auto">
                    ...
                    <span>{{ __('faq.badge') }}</span>
                </div>
                <h2 class="text-2xl sm:text-3xl font-black text-primary">{{ __('faq.heading') }}</h2>
            </div>
            ...
            <div class="space-y-3">
                ...
                <div class="bg-surface rounded-xl border border-gray-100 overflow-hidden hover:border-accent/30 transition-colors">
                    <button @click="active = active === {{ $i+1 }} ? null : {{ $i+1 }}" class="w-full p-4 {{ app()->getLocale()=='en' ? 'text-left' : 'text-right' }} font-bold text-primary flex items-center justify-between gap-3 text-xs sm:text-sm hover:text-accent transition-colors">
                        <span>{{ $faq['q'] }}</span>
                    ...
                    <div x-show="active === {{ $i+1 }}" x-collapse class="px-4 pb-4 text-xs text-gray-600 leading-[1.8] border-t border-gray-100 pt-2.5 {{ app()->getLocale()=='en' ? 'text-left' : 'text-right' }}">{{ $faq['a'] }}</div>
                </div>
                @endforeach
            </div>
        </div>
    ...
    <section class="relative py-16 overflow-hidden">
        ...
        <div class="max-w-4xl mx-auto px-4 text-center space-y-5 relative z-10">
            <h2 class="text-2xl sm:text-3xl lg:text-4xl font-black text-white leading-tight">{{ __('cta.heading') }}</h2>
            <p class="text-medical-200 text-sm max-w-xl mx-auto">{{ __('cta.subtext') }}</p>
