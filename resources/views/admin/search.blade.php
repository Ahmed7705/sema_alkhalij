    ...
    <div class="space-y-6 text-right">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
            <form action="{{ route('admin.search') }}" method="GET" class="flex gap-3">
                <input type="text" name="q" value="{{ $q ?? '' }}" placeholder="ابحث باسم عميل، رقم حجز، رقم فاتورة، اسم منتج أو خدمة، مقالة..." 
        ...
        @if(!empty($q))
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-4">
                    <h3 class="font-black text-sm text-primary flex items-center justify-between">
                        <span>الخدمات الطبية ({{ $services->count() }})</span>
                        ...
                        @forelse($services as $s)
                            <div class="p-3 bg-surface rounded-xl border border-gray-100 flex items-center justify-between text-xs">
                                <div>
                                    ...
                                    <span class="text-gray-500 block">{{ number_format($s->price, 0) }} ر.س • {{ $s->duration_minutes }} دقيقة</span>
                                </div>
                                <a href="{{ route('admin.services.edit', $s->id) }}" class="text-accent font-bold hover:underline text-[11px]">تعديل</a>
                            </div>
                        @empty
                            <p class="text-xs text-gray-400 font-bold py-2">لا توجد نتائج في الخدمات الطبية</p>
                        @endforelse
                    </div>
                </div>
                    ...
                    <div class="space-y-2">
                        @forelse($products as $p)
                            <div class="p-3 bg-surface rounded-xl border border-gray-100 flex items-center justify-between text-xs">
                                ...
                                <a href="{{ route('admin.products.edit', $p->id) }}" class="text-accent font-bold hover:underline text-[11px]">تعديل</a>
                            </div>
                        @empty
                ...
                <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-4">
                    <h3 class="font-black text-sm text-primary flex items-center justify-between">
                        <span>المقالات والمدونة ({{ $articles->count() }})</span>
                    ...
                    <div class="space-y-2">
                        @forelse($articles as $a)
                            <div class="p-3 bg-surface rounded-xl border border-gray-100 flex items-center justify-between text-xs">
                                ...
                                <span class="px-2 py-0.5 rounded bg-indigo-100 text-indigo-700 font-bold text-[10px]">مقال</span>
                            </div>
                        @empty
                            <p class="text-xs text-gray-400 font-bold py-2">لا توجد نتائج في المقالات</p>
                        @endforelse
                    </div>
                </div>
