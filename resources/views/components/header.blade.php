                ...
                @auth
                    @if(in_array(Auth::user()->role, ['admin', 'super_admin', 'manager']))
                        <a href="{{ route('admin.dashboard') }}"
                           ...
                           style="background-color: #006C35 !important;"
                           class="flex items-center gap-2 px-4 py-2.5 text-xs font-black text-white bg-[#006C35] hover:bg-[#00572B] rounded-2xl shadow-md transition-all whitespace-nowrap shrink-0 border border-white/20">
                            ...
                            <span class="hidden sm:inline font-bold">{{ __('nav.admin_panel') }}</span>
                        </a>
                    @elseif(in_array(Auth::user()->role, ['doctor', 'nurse', 'physio', 'lab_tech']))
                        <a href="{{ route('staff.dashboard') }}"
                           ...
                           class="flex items-center gap-2 px-4 py-2.5 text-xs font-black text-white bg-[#006C35] hover:bg-[#00572B] rounded-2xl shadow-md transition-all whitespace-nowrap shrink-0 border border-white/20">
                            ...
                            <span class="hidden sm:inline font-bold">{{ __('nav.staff_portal') }}</span>
                        </a>
                    @elseif(in_array(Auth::user()->role, ['company_admin', 'company_operator']))
                        <a href="{{ route('company.portal') }}"
                           ...
                           class="flex items-center gap-2 px-4 py-2.5 text-xs font-black text-white bg-[#006C35] hover:bg-[#00572B] rounded-2xl shadow-md transition-all whitespace-nowrap shrink-0 border border-white/20">
                            ...
                            <span class="hidden sm:inline font-bold">{{ __('nav.company_portal') }}</span>
                        </a>
                    @endif
                @endauth
