    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-[72px]">
            ...
            <div class="flex items-center gap-1.5 sm:gap-3">
                ...
                {{-- Admin Panel Shortcut (Visible on Mobile & Desktop for Admin) --}}
                @if(Auth::check() && (Auth::user()->role === 'admin' || Auth::user()->isSuperAdmin()))
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-1.5 px-2.5 sm:px-3.5 py-1.5 sm:py-2 text-[11px] sm:text-xs font-black text-white bg-primary hover:bg-primary-hover rounded-xl shadow-sm transition-all border border-white/20" title="لوحة الأدمن">
                        <svg class="w-4 h-4 text-accent" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                        <span>الأدمن</span>
                    </a>
                @endif

                {{-- Login / User Profile Button (Visible on Mobile & Desktop) --}}
                @guest
                    <a href="{{ route('login') }}" class="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs font-bold text-primary bg-medical-50 hover:bg-primary hover:text-white rounded-xl transition-all border border-primary/15" title="تسجيل الدخول">
                ...
                @else
                    <a href="{{ route('profile') }}" class="flex items-center gap-1 px-2 sm:p-2.5 text-gray-700 hover:text-primary hover:bg-medical-50 rounded-xl transition-all" title="حسابي والبروفايل">
                        ...
                        <span class="text-xs font-bold sm:hidden">البروفايل</span>
                    </a>
                @endguest

                {{-- Cart Button (Livewire Reactive - Positioned Next to Profile) --}}
                @livewire('cart-badge')
            ...
            <div x-show="mobileMenuOpen" x-cloak
                ...
                {{-- Mobile Admin & Profile Quick Action Card --}}
                @auth
                    <div class="space-y-2 mb-2">
                        @if(Auth::user()->role === 'admin' || Auth::user()->isSuperAdmin())
                            <a href="{{ route('admin.dashboard') }}" class="w-full flex items-center justify-between p-3 bg-primary text-white rounded-xl font-bold text-sm shadow-md">
                                <div class="flex items-center gap-2">
                                    <svg class="w-5 h-5 text-accent" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                                    <span>لوحة تحكم الأدمن والإدارة</span>
                                </div>
                                <span class="text-xs text-accent font-black">دخول ↗</span>
                            </a>
                        @endif

                        <a href="{{ route('profile') }}" class="w-full flex items-center justify-between p-3 bg-medical-50 text-primary rounded-xl font-bold text-sm border border-primary/10">
                            <div class="flex items-center gap-2">
                                <svg class="w-5 h-5 text-accent" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                <span>البروفايل والملف الطبي ({{ Auth::user()->name }})</span>
                            </div>
                            <span class="text-xs text-accent font-bold">عرض ↗</span>
                        </a>
                    </div>
                @else
                ...
                <div class="pt-3 flex flex-col gap-2">
                    <button @click="callbackModalOpen = true; mobileMenuOpen = false" class="w-full btn-accent py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        طلب خدمة
                    </button>
