    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-[72px]">
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                ...
                {{-- WhatsApp Quick --}}
                <a href="https://wa.me/966545880082" target="_blank" class="hidden xl:flex items-center gap-1.5 px-3 py-2 text-[12px] font-bold text-[#25D366] bg-[#25D366]/10 rounded-lg hover:bg-[#25D366]/20 transition-all" title="واتساب">
                ...
                @if(Auth::check() && (Auth::user()->role === 'admin' || Auth::user()->isSuperAdmin()))
                    <a href="{{ route('admin.dashboard') }}" class="hidden md:flex items-center gap-1.5 px-3 py-2 text-xs font-black text-white bg-primary hover:bg-primary-hover rounded-xl shadow-sm transition-all border border-white/20" title="لوحة الأدمن">
                        <svg class="w-4 h-4 text-accent" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                        <span>لوحة الأدمن</span>
                    </a>
                @endif

                {{-- Prominent Login / Profile Button --}}
                @auth
                    <a href="{{ route('profile') }}" class="inline-flex items-center gap-2 px-3.5 py-2 text-xs font-bold text-primary bg-medical-50 hover:bg-medical-100 rounded-xl transition-all border border-primary/10 shadow-sm" title="البروفايل">
                ...
                @else
                    <a href="{{ route('login') }}" class="inline-flex items-center gap-2 px-3.5 py-2 text-xs font-bold text-primary bg-medical-50 hover:bg-primary hover:text-white rounded-xl transition-all border border-primary/15 shadow-sm group">
                ...
                <button @click="callbackModalOpen = true" class="hidden lg:inline-flex items-center gap-2 px-4 py-2 bg-accent text-white text-xs font-bold rounded-xl shadow-md hover:bg-accent-hover hover:shadow-lg transition-all duration-300">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                    <span>اطلب اتصالاً</span>
                </button>
