...
<body class="bg-gray-50 font-sans text-gray-800 antialiased min-h-screen flex" x-data="{ sidebarOpen: false }">

    {{-- SIDEBAR NAVIGATION --}}
    <aside class="w-64 bg-gradient-to-b from-[#071f18] via-primary to-[#0a3428] text-white flex flex-col shrink-0 min-h-screen border-l border-white/10 shadow-2xl">
        
        {{-- Sidebar Logo Header --}}
        <div class="p-5 border-b border-white/10 flex items-center justify-between">
            <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-white/10 backdrop-blur-md border border-white/20 p-1.5 flex items-center justify-center shrink-0">
        ...
        <div class="p-4 border-t border-white/10">
            <a href="{{ route('home') }}" target="_blank" class="flex items-center justify-between px-4 py-3 rounded-xl bg-white/10 hover:bg-white/20 text-xs font-bold text-white transition-all group">
    ...
    <div class="flex-1 flex flex-col min-w-0">
        ...
        <header class="bg-white border-b border-gray-200 h-16 px-6 flex items-center justify-between shrink-0 shadow-sm">
            <div class="flex items-center gap-4 flex-1 max-w-lg">
                <form action="{{ route('admin.search') }}" method="GET" class="w-full relative">
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="بحث شامل في النظام (عملاء، طلبات، حجوزات، منتجات)..." 
                    ...
                    <svg class="w-4 h-4 text-gray-400 absolute top-3 right-3 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                </form>
            </div>

            <div class="flex items-center gap-4">
                <div class="flex items-center gap-3 text-left">
                    <div class="w-9 h-9 rounded-xl bg-primary text-white font-black text-xs flex items-center justify-center border border-accent/20">
                        {{ mb_substr(auth()->user()->name ?? 'مدير', 0, 2) }}
                    </div>
                    <div class="text-right">
                        <span class="block text-xs font-bold text-gray-800">{{ auth()->user()->name ?? 'مدير النظام' }}</span>
        ...
        {{-- FLASH MESSAGES --}}
        @if(session('success'))
            <div class="p-4 bg-emerald-50 border-b border-emerald-100 text-emerald-800 text-xs font-bold px-6 flex items-center justify-between">
