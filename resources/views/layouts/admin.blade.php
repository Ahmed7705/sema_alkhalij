...
<body class="bg-[#F6F9F7] font-sans text-gray-800 antialiased h-screen overflow-hidden flex" x-data="{ sidebarOpen: false }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'"
           class="fixed inset-y-0 right-0 z-50 w-72 lg:w-64 bg-gradient-to-b from-[#071f18] via-primary to-[#0a3428] text-white flex flex-col shrink-0 h-screen border-l border-white/10 shadow-2xl transition-transform duration-300 ease-in-out lg:sticky lg:top-0 lg:z-auto">
        ...
        <div class="p-5 border-b border-white/10 flex items-center justify-between shrink-0">
            <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3">
                ...
                <div class="space-y-0.5">
                    ...
                    <span class="text-[10px] text-accent font-bold block">لوحة التحكم الإدارية</span>
                </div>
            </a>
        ...
        {{-- Nav Links (Scrollable independently if screen height is small) --}}
        <nav class="p-4 space-y-1.5 flex-1 overflow-y-auto">
            <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all {{ request()->routeIs('admin.dashboard') ? 'bg-accent text-white shadow-md' : 'text-medical-200 hover:bg-white/10 hover:text-white' }}">
                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
                <span>الرئيسية ونظرة عامة</span>
            </a>

            <a href="{{ route('admin.analytics.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all {{ request()->routeIs('admin.analytics.*') ? 'bg-accent text-white shadow-md' : 'text-medical-200 hover:bg-white/10 hover:text-white' }}">
                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                <span>مركز التحليلات والتقارير</span>
            </a>

            <a href="{{ route('admin.services.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all {{ request()->routeIs('admin.services.*') ? 'bg-accent text-white shadow-md' : 'text-medical-200 hover:bg-white/10 hover:text-white' }}">
            ...
            <a href="{{ route('admin.products.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all {{ request()->routeIs('admin.products.*') ? 'bg-accent text-white shadow-md' : 'text-medical-200 hover:bg-white/10 hover:text-white' }}">
            ...
            <a href="{{ route('admin.operations.search') }}" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all {{ request()->routeIs('admin.operations.search') ? 'bg-accent text-white shadow-md' : 'text-medical-200 hover:bg-white/10 hover:text-white' }}">
                ...
                <span>البحث التشغيلي المتقدم</span>
            </a>

            <a href="{{ route('staff.dashboard') }}" target="_blank" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all text-medical-200 hover:bg-white/10 hover:text-white">
                ...
                <span>بوابة الكادر الطبي</span>
            </a>

            <a href="{{ route('company.portal') }}" target="_blank" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all text-medical-200 hover:bg-white/10 hover:text-white">
                ...
                <span>بوابة الشركات المتعاقدة</span>
            </a>

            <a href="{{ route('admin.bookings.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all {{ request()->routeIs('admin.bookings.*') ? 'bg-accent text-white shadow-md' : 'text-medical-200 hover:bg-white/10 hover:text-white' }}">
            ...
            <a href="{{ route('admin.orders.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all {{ request()->routeIs('admin.orders.*') ? 'bg-accent text-white shadow-md' : 'text-medical-200 hover:bg-white/10 hover:text-white' }}">
            ...
            <a href="{{ route('admin.settings.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all {{ request()->routeIs('admin.settings.*') ? 'bg-accent text-white shadow-md' : 'text-medical-200 hover:bg-white/10 hover:text-white' }}">
            ...
            <a href="{{ route('admin.users.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all {{ request()->routeIs('admin.users.*') ? 'bg-accent text-white shadow-md' : 'text-medical-200 hover:bg-white/10 hover:text-white' }}">
            ...
            <a href="{{ route('admin.audit.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold transition-all {{ request()->routeIs('admin.audit.*') ? 'bg-accent text-white shadow-md' : 'text-medical-200 hover:bg-white/10 hover:text-white' }}">
        ...
        <div class="p-4 border-t border-white/10 shrink-0">
            <a href="{{ route('home') }}" target="_blank" class="flex items-center justify-between px-4 py-3 rounded-2xl bg-white/10 hover:bg-white/20 text-xs font-bold text-white transition-all group">
                ...
                <svg class="w-4 h-4 text-accent group-hover:translate-x-[-2px] transition-transform" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
            </a>
        </div>
    </aside>
        ...
        <header class="bg-white/95 backdrop-blur-md border-b border-gray-200/80 h-16 sm:h-20 px-4 sm:px-8 flex items-center justify-between shrink-0 shadow-sm sticky top-0 z-30">
            ...
            <div class="flex items-center gap-3 flex-1 max-w-lg">
                ...
                <form action="{{ route('admin.search') }}" method="GET" class="w-full relative">
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="بحث شامل في النظام..." 
                           class="w-full h-10 sm:h-11 pr-9 sm:pr-10 pl-3 sm:pl-4 bg-gray-50 border border-gray-200/80 rounded-xl sm:rounded-2xl text-xs text-gray-800 font-bold focus:outline-none focus:border-primary transition-all shadow-inner">
                    <svg class="w-4 h-4 text-gray-400 absolute top-3 sm:top-3.5 right-3 sm:right-3.5 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                </form>
            </div>
            ...
            <div class="flex items-center gap-3 shrink-0 mr-2">
                <div class="flex items-center gap-2 sm:gap-3">
                    <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-primary text-white font-black text-xs flex items-center justify-center border border-accent/30 shadow-md shrink-0">
                        {{ mb_substr(auth()->user()->name ?? 'مدير', 0, 2) }}
                    </div>
                    <div class="text-right leading-tight hidden sm:block">
                        <span class="block text-xs font-black text-primary">{{ auth()->user()->name ?? 'مدير النظام' }}</span>
